...
async function settleBuy(supabase, trade, quote, userId, walletPassword) {
    ...
    // Record user TON send in transactions table (non-fatal — blockchain tx already happened)
    try {
        ...
        console.error('[Settle] Non-fatal: failed to insert TON send into transactions:', insertErr.message);
    }
    ...
    try {
        ...
        // Record user NMX receive in transactions table (non-fatal)
        try {
            await supabase.from('transactions').insert({
                user_id: userId,
            ...
            console.error('[Settle] Non-fatal: failed to insert NMX receive into transactions:', insertErr.message);
        }
    } catch (err) {
...
async function settleSell(supabase, trade, quote, userId, walletPassword) {
    ...
    try {
        ...
        // Record user NMX send in transactions table (non-fatal)
        try {
            await supabase.from('transactions').insert({
                user_id: userId,
                wallet_address: userContract.address.toString(),
            ...
            console.error('[Settle] Non-fatal: failed to insert NMX send into transactions:', insertErr.message);
        }
    } catch (nmxErr) {
    ...
    try {
        ...
        // Record user TON receive in transactions table (non-fatal)
        try {
            await supabase.from('transactions').insert({
                user_id: userId,
                wallet_address: userWallet.address,
            ...
            console.error('[Settle] Non-fatal: failed to insert TON receive into transactions:', insertErr.message);
        }
    } catch (err) {
        await supabase.from('treasury_audit_logs').insert({
